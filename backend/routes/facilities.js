...
router.get('/top3', async (req, res) => {
  ...
  res.json(facilities.map(f => ({
    _id: f._id, name: f.name, city: f.city,
    likes: f.likes.length, dislikes: f.dislikes.length,
    sports: f.sports
  })));
});
...
router.post('/', auth, requireRole('employee'), upload.array('images', 10), async (req, res) => {
  try {
...
router.post('/import-json', auth, requireRole('employee'), upload.single('jsonFile'), async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Invalid JSON or server error', error: err.message });
  }
});

// PUT /api/facilities/:id - update facility
router.put('/:id', auth, requireRole('employee'), upload.array('images', 10), async (req, res) => {
  try {
    const facility = await Facility.findOne({ _id: req.params.id, owner: req.user._id });
    if (!facility) return res.status(404).json({ message: 'Facility not found or not yours' });

    const data = JSON.parse(req.body.facilityData || '{}');
    if (req.files?.length) data.images = [...(facility.images || []), ...req.files.map(f => f.filename)];
...
router.post('/:id/react', auth, requireRole('athlete'), async (req, res) => {
  try {
    ...
    const confirmed = await Reservation.countDocuments({
      facility: req.params.id, user: userId, status: 'confirmed'
    });
    if (!confirmed) return res.status(403).json({ message: 'You need at least one confirmed reservation' });

    // Count existing reactions for this user
    const existingComments = await Comment.countDocuments({ facility: req.params.id, user: userId });
    ...
    await facility.save();
    res.json({ likes: facility.likes.length, dislikes: facility.dislikes.length });
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.post('/:id/comment', auth, requireRole('athlete'), async (req, res) => {
  try {
    ...
    if (!confirmed) return res.status(403).json({ message: 'Need confirmed reservation to comment' });
