  ...
  async (req, res) => {
    try {
      ...
      const workingHoursPerDay =
        parseInt(facility.workingHours.close) -
        parseInt(facility.workingHours.open);
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="occupancy-${month}.pdf"`
      );
      ...
      doc
        .fontSize(18)
        .text(`Occupancy Report - ${facility.name}`, { align: "center" });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="equipment-${month}.pdf"`
      );
      ...
      doc
        .fontSize(18)
        .text(`Equipment Sales Report - ${month}`, { align: "center" });
      doc.moveDown();

      let total = 0;
      orders.forEach((order) => {
        doc.text(
          `Order #${order._id} - ${order.user.firstName} ${order.user.lastName} - ${order.totalPrice} RSD`
        );
